...
Before plugging in the Ethernet cable, you can click `Use as LAN` to set the WAN port as a LAN port. That is useful when you are using the router as a [repeater](internet#2-repeater). As a result, you can have two LAN ports which can be used to connect two devices.
